...
use R2Offload\Vendor\Aws\Auth\AuthSelectionMiddleware;
use R2Offload\Vendor\Aws\Auth\AuthSchemeResolverInterface;
use R2Offload\Vendor\Aws\EndpointDiscovery\EndpointDiscoveryMiddleware;
use R2Offload\Vendor\Aws\EndpointV2\EndpointProviderV2;
use R2Offload\Vendor\Aws\EndpointV2\EndpointV2Middleware;
...
class AwsClient implements AwsClientInterface
{
     ...
     *   waiter, or paginator.
     * - credentials:
     *   (R2Offload\Vendor\Aws\Credentials\CredentialsInterface|array|bool|callable) Specifies
     ...
     * - token:
     *   (R2Offload\Vendor\Aws\Token\TokenInterface|array|bool|callable) Specifies
     ...
     * - csm:
     *   (R2Offload\Vendor\Aws\ClientSideMonitoring\ConfigurationInterface|array|callable) Specifies
     *   the credentials used to sign requests. Provide an
     ...
     *   of S3).
     * - endpoint_discovery: (R2Offload\Vendor\Aws\EndpointDiscovery\ConfigurationInterface,
     ...
     *   available regions.
     * - retries: (int, R2Offload\Vendor\Aws\Retry\ConfigurationInterface, R2Offload\Vendor\Aws\CacheInterface,
    ...
    private function parseClass()
    {
        ...
        return [
            strtolower($service),
            "R2Offload\\Vendor\\Aws\\{$service}\\Exception\\{$service}Exception"
        ];
    }
    ...
    private function loadAliases($file = null)
    {
        if (!isset($this->aliases)) {
            ...
            $aliases = \R2Offload\Vendor\Aws\load_compiled_json($file);
    ...
    {
        $aliases = \R2Offload\Vendor\Aws\load_compiled_json(__DIR__ . '/data/aliases.json');
