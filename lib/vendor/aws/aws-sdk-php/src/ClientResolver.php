...
use R2Offload\Vendor\Aws\Auth\AuthResolver;
use R2Offload\Vendor\Aws\Auth\AuthSchemeResolver;
use R2Offload\Vendor\Aws\Auth\AuthSchemeResolverInterface;
use R2Offload\Vendor\Aws\ClientSideMonitoring\ApiCallAttemptMonitoringMiddleware;
use R2Offload\Vendor\Aws\ClientSideMonitoring\ApiCallMonitoringMiddleware;
use R2Offload\Vendor\Aws\ClientSideMonitoring\Configuration;
use R2Offload\Vendor\Aws\Configuration\ConfigurationResolver;
use R2Offload\Vendor\Aws\Credentials\CredentialProvider;
use R2Offload\Vendor\Aws\Credentials\Credentials;
use R2Offload\Vendor\Aws\Credentials\CredentialsInterface;
use R2Offload\Vendor\Aws\DefaultsMode\ConfigurationInterface as ConfigModeInterface;
use R2Offload\Vendor\Aws\DefaultsMode\ConfigurationProvider as ConfigModeProvider;
use R2Offload\Vendor\Aws\Endpoint\EndpointProvider;
use R2Offload\Vendor\Aws\Endpoint\PartitionEndpointProvider;
use R2Offload\Vendor\Aws\Endpoint\UseDualstackEndpoint\Configuration as UseDualStackEndpointConfiguration;
use R2Offload\Vendor\Aws\Endpoint\UseDualstackEndpoint\ConfigurationInterface as UseDualStackEndpointConfigurationInterface;
use R2Offload\Vendor\Aws\Endpoint\UseDualstackEndpoint\ConfigurationProvider as UseDualStackConfigProvider;
use R2Offload\Vendor\Aws\Endpoint\UseFipsEndpoint\Configuration as UseFipsEndpointConfiguration;
use R2Offload\Vendor\Aws\Endpoint\UseFipsEndpoint\ConfigurationInterface as UseFipsEndpointConfigurationInterface;
use R2Offload\Vendor\Aws\Endpoint\UseFipsEndpoint\ConfigurationProvider as UseFipsConfigProvider;
use R2Offload\Vendor\Aws\EndpointDiscovery\ConfigurationInterface;
use R2Offload\Vendor\Aws\EndpointDiscovery\ConfigurationProvider;
...
use R2Offload\Vendor\Aws\Retry\ConfigurationInterface as RetryConfigInterface;
use R2Offload\Vendor\Aws\Retry\ConfigurationProvider as RetryConfigProvider;
use R2Offload\Vendor\Aws\Signature\SignatureProvider;
use R2Offload\Vendor\Aws\Token\Token;
use R2Offload\Vendor\Aws\Token\TokenInterface;
use R2Offload\Vendor\Aws\Token\BedrockTokenProvider;
...
class ClientResolver
{
    ...
    private static $defaultArgs = [
        'service' => [
            'type'     => 'value',
            'valid'    => ['string'],
            'doc'      => 'Name of the service to utilize. This value will be supplied by default when using one of the SDK clients (e.g., R2Offload\\Vendor\\Aws\\S3\\S3Client).',
            'required' => true,
            'internal' => true
        ],
        'exception_class' => [
        ...
        'csm' => [
            'type'     => 'value',
            'valid'    => [\R2Offload\Vendor\Aws\ClientSideMonitoring\ConfigurationInterface::class, 'callable', 'array', 'bool'],
            ...
            'default'  => [\R2Offload\Vendor\Aws\ClientSideMonitoring\ConfigurationProvider::class, 'defaultProvider']
        ],
        'http' => [
            'type'    => 'value',
            'valid'   => ['array'],
    ...
    public static function _apply_csm($value, array &$args, HandlerList $list)
    {
        if ($value === false) {
            $value = new Configuration(
                false,
                \R2Offload\Vendor\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_HOST,
                \R2Offload\Vendor\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_PORT,
                \R2Offload\Vendor\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_CLIENT_ID
            );
            $args['csm'] = $value;
        }
    ...
    public static function _apply_endpoint_provider($value, array &$args)
    {
        if (!isset($args['endpoint'])) {
            if ($value instanceof \R2Offload\Vendor\Aws\EndpointV2\EndpointProviderV2) {
            ...
            $args['region'] = \R2Offload\Vendor\Aws\strip_fips_pseudo_regions($args['region']);
    ...
    public static function _default_endpoint_provider(array $args)
    {
        ...
        if (self::isValidService($serviceName)
            ...
            return new \R2Offload\Vendor\Aws\EndpointV2\EndpointProviderV2(
                $ruleset,
    ...
    public static function _apply_suppress_php_deprecation_warning($value, &$args)
    {
        if ($value)  {
            ...
            $args['suppress_php_deprecation_warning']
                = \R2Offload\Vendor\Aws\boolean_value(getenv("AWS_SUPPRESS_PHP_DEPRECATION_WARNING"));
        } elseif (!empty($_SERVER["AWS_SUPPRESS_PHP_DEPRECATION_WARNING"])) {
            $args['suppress_php_deprecation_warning'] =
                \R2Offload\Vendor\Aws\boolean_value($_SERVER["AWS_SUPPRESS_PHP_DEPRECATION_WARNING"]);
        } elseif (!empty($_ENV["AWS_SUPPRESS_PHP_DEPRECATION_WARNING"])) {
            $args['suppress_php_deprecation_warning'] =
                \R2Offload\Vendor\Aws\boolean_value($_ENV["AWS_SUPPRESS_PHP_DEPRECATION_WARNING"]);
        }
    }
    ...
    public static function _default_endpoint(array &$args)
    {
        ...
        $serviceIdentifier = \R2Offload\Vendor\Aws\manifest($args['service'])['serviceIdentifier'];
...
EOT;
    ...
    private static function isValidService($service)
    {
        ...
        $services = \R2Offload\Vendor\Aws\manifest();
