...
use R2Offload\Vendor\Aws\Api\DateTimeResult;
use R2Offload\Vendor\Aws\CacheInterface;
use R2Offload\Vendor\Aws\Exception\CredentialsException;
use R2Offload\Vendor\Aws\Sts\StsClient;
...
class CredentialProvider
{
     ...
     * @see R2Offload\Vendor\Aws\Credentials\InstanceProfileProvider for $config details.
     */
     ...
     * @see R2Offload\Vendor\Aws\Credentials\EcsCredentialProvider for $config details.
     */
     ...
     * @param array $config Array of configuration data
     * @return callable
     * @see R2Offload\Vendor\Aws\Credentials\AssumeRoleCredentialProvider for $config details.
     */
     ...
     * @param array $config Array of configuration data
     * @return callable
     * @see R2Offload\Vendor\Aws\Credentials\AssumeRoleWithWebIdentityCredentialProvider for
     * $config details.
     */
    ...
    public static function process($profile = null, $filename = null)
    {
        ...
        return function () use ($profile, $filename) {
            ...
            $data = \R2Offload\Vendor\Aws\parse_ini_file($filename, true, INI_SCANNER_RAW);
            if ($data === false) {
                return self::reject("Invalid credentials file: $filename");
            }
            if (!isset($data[$profile])) {
    ...
    public static function loadProfiles($filename)
    {
        ...
        if ($filename === self::getHomeDir() . '/.aws/credentials'
            ...
            $configProfileData = \R2Offload\Vendor\Aws\parse_ini_file($configFilename, true, INI_SCANNER_RAW);
    ...
    private static function loadDefaultProfiles()
    {
        ...
        if (file_exists($credFile)) {
            $profiles = \R2Offload\Vendor\Aws\parse_ini_file($credFile, true, INI_SCANNER_RAW);
        }
    ...
    private static function getSsoCredentials($profiles, $ssoProfileName, $filename, $config)
    {
        if (empty($config['ssoOidcClient'])) {
            ...
            $ssoOidcClient = new \R2Offload\Vendor\Aws\SSOOIDC\SSOOIDCClient([
        ...
        $tokenPromise = new \R2Offload\Vendor\Aws\Token\SsoTokenProvider(
            $ssoProfileName,
            $filename,
            $ssoOidcClient
        );
    ...
    private static function getCredentialsFromSsoService($ssoProfile, $clientRegion, $accessToken, $config)
    {
        if (empty($config['ssoClient'])) {
            $ssoClient = new \R2Offload\Vendor\Aws\SSO\SSOClient([
                'region' => $clientRegion,
                'version' => '2019-06-10',
                'credentials' => false
            ]);
