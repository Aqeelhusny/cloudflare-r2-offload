...
use R2Offload\Vendor\Aws\Exception\InvalidJsonException;
use R2Offload\Vendor\Aws\Sdk;
use R2Offload\Vendor\GuzzleHttp\Exception\TransferException;
...
class InstanceProfileProvider
{
    ...
    public function __construct(array $config = [])
    {
        ...
        $this->client = $config['client'] ?? \R2Offload\Vendor\Aws\default_http_handler();
    ...
    private function request($url, $method = 'GET', $headers = [])
    {
        ...
        $userAgent .= ' ' . \R2Offload\Vendor\Aws\default_user_agent();
    ...
    {
        $isImdsV1Disabled = \R2Offload\Vendor\Aws\boolean_value($this->ec2MetadataV1Disabled)
            ?? \R2Offload\Vendor\Aws\boolean_value(
